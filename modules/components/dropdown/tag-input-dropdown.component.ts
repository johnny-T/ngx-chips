...
import { TagModel, TagModelClass } from "../../core/accessor";
...
export class TagInputDropdown implements AfterViewInit {
  ...
  @Input() public get autocompleteItems(): TagModel[] {
    ...
    return items.map((item: TagModel, index: number) => {
  ...
  public show = (): void => {
    ...
    console.log(items);
    this.setItems(items as TagModelClass[]);
   ...
   * @name setItems
   */
  private setItems(items: TagModelClass[]): void {
    let finalItems = [];
    const categoryItems = [];
  ...
  private getItemsFromObservable = (text: string): void => {
    ...
    const subscribeFn = (data: any[]) => {
      ...
      this.setItems(this.getMatchingItems(text) as TagModelClass[]);
